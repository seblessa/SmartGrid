...
class ChronoCartographerAgent(Agent):
    ...
    async def setup(self):
        ...
        def start_pygame():
            ...
            self.add_behaviour(self.DrawMap())
            pygame.display.flip()
            clock.tick(60)
    ...
    class TimeUpdate(PeriodicBehaviour):
        async def run(self):
            ...
            agent_list = [
                "wind_energy_generator@localhost",
                "solar_energy_generator@localhost",
                "hydro_energy_generator@localhost",
            ]
            for agent_jid in agent_list:
                msg = Message(to=agent_jid)
                msg.body = self.agent.time
                await self.send(msg)
    ...
    class DrawMap(CyclicBehaviour):
        async def run(self):
            ...
            balance_text = font.render(f"{self.agent.env.get_balance()}", True, BLACK)
            ...
            for i, structure in enumerate(
                    [self.agent.env.get_hospital(), self.agent.env.get_police_department(),
                     self.agent.env.get_fire_department()]):
                base_radius = 35
            ...
            for i, neighborhood in enumerate(self.agent.env.get_neighborhoods()):
                x = int(screen_size[0] / 2 + 400 * pygame.math.Vector2(1, 0).rotate(
                    i * 360 / len(self.agent.env.get_neighborhoods())).x)
                y = int(screen_size[1] / 2 + 250 * pygame.math.Vector2(1, 0).rotate(
                    i * 360 / len(self.agent.env.get_neighborhoods())).y)
                ...
                for j, (house_x, house_y) in enumerate(house_positions):
                    ...
                    pygame.draw.circle(screen, GREEN if neighborhood.houses[j].online else RED,
            ...
            # Draw solar panel
            solar_stations = self.agent.env.get_solar_panels()
            ...
            gen_text = font.render(f"Generating: {self.agent.env.get_solar_generation()}", True, BLACK)
            ...
            # Draw wind stations
            for i, station in enumerate(self.agent.env.get_wind_turbines()):
            ...
            # Draw hydro station
            hydro_stations = self.agent.env.get_hydro_generator()
            if hydro_stations:
            ...
            fossil_fuel_station = self.agent.env.get_ff_station()
